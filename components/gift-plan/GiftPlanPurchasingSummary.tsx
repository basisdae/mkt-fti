...
export function GiftPlanPurchasingSummary({
  ...
  const rows = buildPurchasingSummary(tiers);
        ...
        <table className="min-w-full text-left text-xs">
          <thead>
            <tr className="border-b border-gray-100 text-gray-500">
              ...
              <th className="px-2 py-2 font-medium">{t.purchasingRequiredQty}</th>
              <th className="px-2 py-2 font-medium">{t.purchasingUnitCost}</th>
              <th className="px-2 py-2 font-medium">{t.purchasingTotalCost}</th>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.group_key} className="border-b border-gray-50">
                ...
                <td className="px-2 py-2">{formatGiftQty(row.total_required_qty)}</td>
                <td className="px-2 py-2">
                  {row.unit_actual_cost === "mixed"
                    ? t.purchasingMixed
                    : formatGiftMoney(row.unit_actual_cost)}
                </td>
                <td className="px-2 py-2">{formatGiftMoney(row.total_actual_cost)}</td>
